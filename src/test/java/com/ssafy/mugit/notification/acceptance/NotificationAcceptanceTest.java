...
public class NotificationAcceptanceTest {
    ...
    @Test
    @DisplayName("[인수] 알림 읽기 실패(404)")
    void testReadNotification404() throws Exception {
        // given
        Cookie[] myLoginCookie = mockMvc.perform(get("/api/users/login").header(HttpHeaders.AUTHORIZATION, "Bearer valid_token")).andReturn().getResponse().getCookies();

        // when
        ResultActions perform = mockMvc.perform(patch("/api/users/notifications/-1").cookie(myLoginCookie));
    ...
    @Test
    @Transactional
    @DisplayName("[인수] 알림 읽기 성공(200)")
    void testReadNotification200() throws Exception {
        // given
        Cookie[] followingUserLoginCookie = mockMvc.perform(get("/api/users/login").header(HttpHeaders.AUTHORIZATION, "Bearer valid_token_2")).andReturn().getResponse().getCookies();
        Cookie[] myLoginCookie = mockMvc.perform(get("/api/users/login").header(HttpHeaders.AUTHORIZATION, "Bearer valid_token")).andReturn().getResponse().getCookies();
        ...
        ResultActions perform = mockMvc.perform(patch("/api/users/notifications/" + notificationId).cookie(myLoginCookie));
